...
function checkIfPlayerWon(
  ...
  return DIRECTION_DELTAS.some(([deltaRow, deltaCol]) => {
    // Count consecutive discs in both directions from the current position
    let count = 1; // Start with 1 for the current disc
...
function GameGrid({ grid }: { grid: GameGridType }): React.ReactElement {
  return (
    <div
      className="grid"
      style={{
...
function WinnerSection({ winner }: { winner: Player }): React.ReactElement {
  return (
    <div
...
export default function App() {
  ...
  function onPlayerMove(column: number) {
    ...
    while (rowToPlace >= 0 && newGrid[rowToPlace][column] != null) {
      rowToPlace--;
    }

    if (rowToPlace < 0) return; // column full
